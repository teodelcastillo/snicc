...
* all other parameters are for email authentification. Email is automatically sent by the server for registering and login.

## Load data

For a clean re-install, first stop the container and delete the database.
...
Load media files : unzip the `media.zip` archive in the `app` directory. Restart nginx :
...
Log into the admin dashboard at the `/admin/` path, for instance `http://localhost/admin/` depending on your configuration.
